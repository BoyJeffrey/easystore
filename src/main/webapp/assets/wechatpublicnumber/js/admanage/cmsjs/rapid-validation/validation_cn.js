...
Validator.messageSource['en-us'] = [
	...
	['validate-one-required-byClassName' , 'Please select one of the above options.'],
...
Validator.messageSource['zh-cn'] = [
	...
	['validate-one-required-byClassName' , '在前面选项至少选择一个.'],
...
Object.extend(Validation, {
	invalidCharTypes:["INPUT", "TEXTAREA"],
	...
	validateElement : function(elm, options){
		...
		if(elm.type!="hidden" &&Validation.arrayContain(Validation.invalidCharTypes, elm.tagName)){
			var result = Validation.test('invalid-char',elm,options.useTitle);
...
Validation.addAllThese([
	...
	['validate-one-required-byClassName', function (v,elm) {
		//var p = elm.parentNode;
		//var p = ValidationUtils.getReferenceForm(elm);
		var options = document.getElementsByClassName(elm.id);
		return $A(options).any(function(elm) {
			return $F(elm);
		});
	},{ignoreEmptyValue : false}],
