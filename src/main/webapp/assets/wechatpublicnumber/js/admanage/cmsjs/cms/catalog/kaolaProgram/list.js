var mams_contentpic={		
		init:function(){$("#catalogList").jqGrid({
		   	url:ctx +'/catalog/kaolaProgram/list/forJson.do',
			datatype: "json",
			jsonReader:{
					root: "result",
					page: "pageBean.page",
...
function deleteOne(id) {
	if(id!=null){
		$.ajax({
			type : "get",
			url : ctx+"/catalog/kaolaProgram/delete.do?ids="+id,
			data : "",
			dataType : "json",
			async : true,
			cache : false,
...
function suspendOne(id) {
	if(id!=null){
		myDialog.showConfirm("此操作会暂停此分类及其子分类，您确定吗？",function(){
			$.ajax({
				type : "get",
				url : ctx+"/catalog/kaolaProgram/suspend.do?ids="+id,
				data : "",
				dataType : "json",
				async : true,
				cache : false,
...
function valid(id) {
	if(id!=null){
		myDialog.showConfirm("此操作会启用此分类及其子分类，您确定吗？",function(){
			$.ajax({
				type : "get",
				url : ctx+"/catalog/kaolaProgram/valid.do?ids="+id,
				data : "",
				dataType : "json",
				async : true,
				cache : false,
...
function move(ids){
	if(ids!=null){
		myDialog.showConfirm("此操作会改变分类显示顺序，您确定吗？",function(){
			$.ajax({
				type : "get",
				url : ctx+"/catalog/kaolaProgram/move.do?ids="+ids,
				data : "",
				dataType : "json",
				async : true,
				cache : false,
...
function showdiary(obj,dataId){
	var url = ctx+"/catalog/kaolaProgram/operations.do?dataId="+dataId;
...
function hasChild(id){
	...
	if(flag.indexOf("+")>=0){		
		...
		if(id!=null){			
			$.ajax({
				type : "get",
				url : ctx+"/catalog/kaolaProgram/getChildren.do?parentId="+id,
				data : "",
				dataType : "json",
				async : true,
				cache : false,
		...
		$.ajax({
			type : "get",
			url : ctx+"/catalog/kaolaProgram/getDescendant.do?parentId="+id,
			data : "",
			dataType : "json",
			async : true,
			cache : false,
