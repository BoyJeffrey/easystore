...
import cn.usually.common.pay.union.sdknew.AcpService;
import cn.usually.common.pay.union.sdknew.DemoBase;
...
public class UnionService extends Service {
	...
	public void pay(PayParam payParam, HttpServletResponse res, CheckInfo checkInfo) {
		// 查询银联配置信息
		Union_config unionConfig = dao().fetch(Union_config.class);
        if(unionConfig != null) {
        	if(Strings.isNullOrEmpty(unionConfig.getMerId()) || Strings.isNullOrEmpty(unionConfig.getSignCert_pwd())
        			|| Strings.isNullOrEmpty(unionConfig.getNotify_url())) {
        		checkInfo.setFlag(false);
        		checkInfo.setMsg("银联账户配置有误");
        		return;
        	}else {
        		SDKConfig.setSignCertPwd(unionConfig.getSignCert_pwd());
        	}
        }else {
        	checkInfo.setFlag(false);
        	checkInfo.setMsg("银联账户配置有误");
		...
		requestData.put("orderId", payParam.getOut_trade_no()); // 商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则
		requestData.put("txnTime", DemoBase.getCurrentTime()); // 订单发送时间，取系统时间，格式为YYYYMMDDhhmmss，必须取当前时间，否则会报txnTime无效
		requestData.put("currencyCode", "156"); // 交易币种（境内商户一般是156 人民币）
		requestData.put("txnAmt", String.valueOf((int) (payParam.getAmount() * 100))); // 交易金额，单位分，不要带小数点
		...
		requestData.put("frontUrl", payParam.getFront_url());
		...
		requestData.put("backUrl", unionConfig.getNotify_url());
		// ////////////////////////////////////////////////
		//
		...
		String sHtmlText = AcpService.createAutoFormHtml(requestFrontUrl, submitFromData, DemoBase.encoding_UTF8); // 生成自动跳转的Html表单
		try {
